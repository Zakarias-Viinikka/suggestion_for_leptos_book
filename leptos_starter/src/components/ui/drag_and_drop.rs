...
#[component]
pub fn DraggableZone(#[prop(into, optional)] class: String, children: Children) -> impl IntoView {
    let merged = tw_merge::tw_merge!("dragabble__container bg-neutral-600 p-4 mt-4", class);
    view! {
        <div class=merged data-name="DraggableZone">
            {children()}
        </div>
    }
}
...
#[component]
pub fn DraggableItem(children: Children) -> impl IntoView {
    view! {
        <div
...
fn get_drag_after_element(container: &Element, y: f64) -> Option<HtmlElement> {
    ...
    for i in 0..items.length() {
        ...
        let rect: web_sys::DomRect = el.get_bounding_client_rect();
